...
let collectionsIndexesPromise: Promise<any> | undefined;

export async function getCollectionsCollection(): Promise<Collection<Document>> {
  const database = await getDatabase();
  const collection = database.collection("collections");

  if (!collectionsIndexesPromise) {
    collectionsIndexesPromise = Promise.all([
      collection.createIndex({ slug: 1 }, { unique: true }),
      collection.createIndex({ name: 1 }),
      collection.createIndex({ updatedAt: -1 }),
    ]).catch((err) => {
      console.error("Failed to create collections indexes:", err);
    });
  }
  await collectionsIndexesPromise;
...
export async function listCollectionsForSelect(): Promise<CollectionSelectItem[]> {
  const collection = await getCollectionsCollection();
  const rows = await collection
...
export async function findCollectionSummaryById(collectionId: string): Promise<CollectionSummary | null> {
  const collection = await getCollectionsCollection();
...
export async function updateCollection(collectionId: string, input: unknown): Promise<{ id: string; slug: string }> {
  const payload = collectionSchema.parse(input);
  const collection = await getCollectionsCollection();
...
export function parseCollectionPayload(formData: FormData): CollectionPayload {
