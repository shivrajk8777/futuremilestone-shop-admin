...
import { DiscountDetail } from "../../../lib/discounts";
import { DiscountActionState } from "./actions";

const initialState: DiscountActionState = { error: "" };

function normalizeInitialDiscount(discount?: DiscountDetail | null) {
  return {
    name: discount?.name ?? "",
...
export interface DiscountFormProps {
  action: (state: DiscountActionState, formData: FormData) => Promise<DiscountActionState>;
  ...
  return (
    <form action={formAction} className="grid gap-3">
      ...
      <section className="p-[18px] sm:p-[22px] bg-futuremilestone-panel/72 border border-futuremilestone-soft-line backdrop-blur-[14px] rounded-[32px] shadow-futuremilestone-soft">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="grid gap-2.5">
            ...
            <div className="flex items-center gap-3 h-[54px] border border-futuremilestone-ink/10 rounded-[18px] bg-futuremilestone-input-bg px-[18px] select-none">
              <input
                id="active"
                onChange={(event) => updateField("active", event.target.checked)}
                type="checkbox"
                checked={form.active}
                className="w-4 h-4 rounded text-futuremilestone-accent focus:ring-futuremilestone-accent cursor-pointer"
              />
              <span className="text-[14px] font-medium text-futuremilestone-ink cursor-pointer" onClick={() => updateField("active", !form.active)}>
          ...
          {form.scope === "category" && (
            <div className="grid gap-2.5 md:col-span-2 bg-futuremilestone-panel/40 border border-futuremilestone-soft-line rounded-[22px] p-4">
              ...
              <input
                ...
                className="w-full border border-futuremilestone-ink/10 rounded-full px-4 py-2 bg-futuremilestone-panel-strong outline-none text-[13px]"
              />
              <div className="max-h-60 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 p-1">
                {filteredCollections.length === 0 ? (
                  <div className="text-futuremilestone-muted text-xs p-2">No matching collections found</div>
                ) : (
                    ...
                    return (
                      <label
                        ...
                        className={`flex items-center gap-3 p-3 rounded-[14px] border cursor-pointer select-none transition-all ${
                          isChecked
                            ? "bg-futuremilestone-accent-soft/20 border-futuremilestone-accent/30 text-futuremilestone-ink"
                            : "bg-futuremilestone-panel-strong border border-futuremilestone-soft-line hover:bg-futuremilestone-accent-soft text-futuremilestone-muted"
                        }`}
                      >
                        <input
                          type="checkbox"
                          ...
                          className="w-4 h-4 rounded text-futuremilestone-accent focus:ring-futuremilestone-accent"
                        />
            ...
            <div className="grid gap-2.5 md:col-span-2 bg-futuremilestone-panel/40 border border-futuremilestone-soft-line rounded-[22px] p-4">
              ...
              <div className="max-h-60 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 p-1">
                {filteredProducts.length === 0 ? (
                  <div className="text-futuremilestone-muted text-xs p-2">No matching products found</div>
                ) : (
      ...
      <div className="flex justify-end mt-2">
        <button className="rounded-full px-6 py-3 border border-transparent bg-futuremilestone-accent text-futuremilestone-bg font-semibold text-center transition hover:bg-opacity-90 active:scale-[0.98] cursor-pointer text-[14px]" disabled={isPending} type="submit">
