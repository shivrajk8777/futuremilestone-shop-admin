...
import { DiscountItem } from "../../../lib/discounts";
...
function formatScope(scope: string, collectionIds?: string[], productIds?: string[]): string {
  if (scope === "all") {
    return "All Products";
  }
  if (scope === "category") {
...
export default function DiscountList({ discounts }: DiscountListProps) {
  ...
  if (!discounts.length) {
    return (
      <div className="bg-futuremilestone-panel-strong border border-futuremilestone-soft-line rounded-[22px] p-8 text-center">
        ...
        <p className="mt-1 text-futuremilestone-muted text-[13px]">Create the first discount campaign to set price promotions.</p>
      </div>
    );
  }
        ...
        <table className="w-full border-collapse text-left text-[13px]">
          ...
          <tbody className="divide-y divide-futuremilestone-soft-line/60">
            {paginatedDiscounts.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-5 py-8 text-center text-futuremilestone-muted">
                  No matching campaigns found.
                </td>
              </tr>
            ) : (
              paginatedDiscounts.map((discount) => (
                <tr key={discount.id} className="hover:bg-futuremilestone-accent/2 transition-colors animate-fade-in">
                  <td className="px-5 py-3 font-semibold text-futuremilestone-ink">{discount.name}</td>
                  <td className="px-5 py-3 font-medium text-futuremilestone-ink">
                  ...
                  <td className="px-5 py-3">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-semibold ${
                      discount.active
                        ...
                        : "text-futuremilestone-muted bg-futuremilestone-panel-strong/80"
                    }`}>
                      <span className="w-1.5 h-1.5 rounded-full bg-current" />
                      {discount.active ? "Active" : "Inactive"}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-futuremilestone-muted">{formatDate(discount.updatedAt)}</td>
