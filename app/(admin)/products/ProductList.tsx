...
export default function ProductList({ products: initialProducts, collections }: ProductListProps) {
  ...
  const handleToggleFavorite = async (productId: string, currentFavorite: boolean) => {
  ...
  if (!initialProducts.length) {
    return (
      <div className="bg-futuremilestone-panel-strong border border-futuremilestone-soft-line rounded-[22px] p-8 text-center">
        ...
        <p className="mt-1 text-futuremilestone-muted text-[13px]">Create the first product to start building the live catalog.</p>
      </div>
    );
  }
  ...
  if (isReordering) {
    return (
      <div className="bg-futuremilestone-panel-strong border border-futuremilestone-soft-line rounded-[24px] overflow-hidden shadow-futuremilestone-soft animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-4 border-b border-futuremilestone-soft-line bg-futuremilestone-bg/20">
          <div>
            <h3 className="m-0 text-[15px] font-bold text-futuremilestone-ink font-semibold">Rearrange Products: {selectedCategoryName}</h3>
            <p className="m-0 mt-0.5 text-futuremilestone-muted text-[12px]">Drag and drop products to rearrange their order within this view.</p>
          </div>
        ...
        <div className="p-6 grid gap-2.5 max-w-xl mx-auto py-8">
          {orderedList.length === 0 ? (
            <div className="text-center text-futuremilestone-muted py-8">
            ...
            orderedList.map((product, index) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <span className="font-semibold text-futuremilestone-ink block truncate">{product.name}</span>
                  <span className="text-futuremilestone-muted text-[11px] block truncate font-medium">
                    {product.materialCount} materials • {product.dimensionCount} dimensions
                  </span>
                </div>

                <div className="text-right flex-shrink-0">
                  <span className="font-semibold text-futuremilestone-ink block text-[13px]">{formatPrice(product.startingPrice)}</span>
                </div>
              </div>
            ))
          )}
  ...
  return (
    <div className="bg-futuremilestone-panel-strong border border-futuremilestone-soft-line rounded-[24px] overflow-hidden shadow-futuremilestone-soft animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-3 p-4 border-b border-futuremilestone-soft-line bg-futuremilestone-bg/20">
        <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-2 text-[13px] text-futuremilestone-muted">
            ...
            <select
              ...
              className="bg-futuremilestone-panel-strong border border-futuremilestone-soft-line rounded-lg px-2.5 py-1.5 text-futuremilestone-ink font-medium focus:outline-none max-w-[200px]"
            >
        ...
        <table className="w-full border-collapse text-left text-[13px]">
          ...
          <tbody className="divide-y divide-futuremilestone-soft-line/60">
            {paginatedProducts.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-5 py-8 text-center text-futuremilestone-muted">
                  No matching products found.
                </td>
              </tr>
            ) : (
              paginatedProducts.map((product) => (
                <tr key={product.id} className="hover:bg-futuremilestone-accent/2 transition-colors animate-fade-in">
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      {product.imageUrl ? (
                        <a
                          href={`${process.env.NEXT_PUBLIC_STORE_URL || 'https://futuremilestone.shop'}/shop/${product.slug}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          title="View on website"
                          className="hover:opacity-80 transition-opacity"
                        >
                          <img alt={product.name} className="w-9 h-9 rounded-lg object-cover bg-futuremilestone-ink/8 border border-futuremilestone-soft-line flex-shrink-0 cursor-pointer" src={product.imageUrl} />
                        </a>
                      ) : (
                  ...
                  <td className="px-5 py-3 text-futuremilestone-muted">{product.collectionName || "Unassigned"}</td>
                  <td className="px-5 py-3 text-futuremilestone-muted font-mono">/{product.slug}</td>
                  <td className="px-5 py-3 text-futuremilestone-muted">
                    {product.materialCount} materials • {product.dimensionCount} dimensions
                  </td>
                  <td className="px-5 py-3 font-semibold text-futuremilestone-ink">{formatPrice(product.startingPrice)}</td>
                  
                  {/* Heart button cell */}
                  <td className="px-5 py-3 text-center">
                    <button
                      onClick={() => handleToggleFavorite(product.id, product.favorite)}
                      className="inline-flex items-center justify-center p-1.5 rounded-full hover:bg-red-500/10 active:scale-90 transition-all text-futuremilestone-muted hover:text-red-500 cursor-pointer"
                  ...
                  <td className="px-5 py-3 text-futuremilestone-muted">{formatDate(product.updatedAt)}</td>
