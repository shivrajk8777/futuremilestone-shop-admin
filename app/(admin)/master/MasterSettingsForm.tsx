...
export interface MasterSettingsFormProps {
  initialSettings: MasterSettings;
  collections?: CollectionSelectItem[];
  action: (state: MasterActionState, formData: FormData) => Promise<MasterActionState>;
}

export default function MasterSettingsForm({ initialSettings, collections = [], action }: MasterSettingsFormProps) {
  const [state, formAction, isPending] = useActionState(action, initialState);
  const [form, setForm] = useState<MasterSettings>(initialSettings);
  ...
  const payload = useMemo(
    () =>
      JSON.stringify({
        ...
        slides: form.slides.map((slide: CarouselSlide) => ({
          slug: slide.slug,
          name: slide.name,
  ...
  function updateField(field: string, value: any) {
    setForm((current: MasterSettings) => ({ ...current, [field]: value }));
  }

  function updateSlide(idx: number, field: string, value: any) {
  ...
  function handleDragEnter(e: DragEvent<HTMLDivElement>, idx: number) {
    e.preventDefault();
    e.stopPropagation();
  ...
  async function handleDrop(e: DragEvent<HTMLDivElement>, idx: number) {
    e.preventDefault();
    e.stopPropagation();
    setDraggingIndex(null);
      ...
      <section className="p-[18px] sm:p-[22px] bg-futuremilestone-panel/72 border border-futuremilestone-soft-line backdrop-blur-[14px] rounded-[32px] shadow-futuremilestone-soft">
        <div className="flex items-center justify-between gap-4 mb-4">
          <div>
            ...
            <p className="mt-1 mb-0 text-futuremilestone-muted text-[13px]">
          ...
          <button
            ...
            className={`relative inline-flex h-6.5 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
              form.marqueeVisible ? "bg-futuremilestone-accent" : "bg-futuremilestone-ink/10"
            }`}
          >
            <span
              className={`pointer-events-none inline-block h-5.5 w-5.5 transform rounded-full bg-futuremilestone-panel-strong shadow ring-0 transition duration-200 ease-in-out ${
        ...
        {form.marqueeVisible && (
          <div className="grid gap-2 mt-2">
            <label className="text-[13px] font-semibold text-futuremilestone-ink" htmlFor="marqueeText">
        ...
        <div className="flex items-center justify-between gap-4 mb-4">
          <div>
            <h2 className="mt-1 mb-0 text-[22px] font-bold tracking-[-0.04em] flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-futuremilestone-accent/15 text-futuremilestone-accent">
          ...
          <button
            ...
            className={`relative inline-flex h-6.5 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
              (form.firstOrderDiscountVisible ?? true) ? "bg-futuremilestone-accent" : "bg-futuremilestone-ink/10"
            }`}
          >
            <span
              className={`pointer-events-none inline-block h-5.5 w-5.5 transform rounded-full bg-futuremilestone-panel-strong shadow ring-0 transition duration-200 ease-in-out ${
        ...
        {(form.firstOrderDiscountVisible ?? true) && (
          <div className="grid gap-3 mt-2">
            <div className="grid gap-2">
              <label className="text-[13px] font-semibold text-futuremilestone-ink" htmlFor="firstOrderDiscountPercentage">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="20"
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-futuremilestone-muted font-bold text-[14px] pointer-events-none">
                  %
                </span>
              </div>
            </div>
            <div className="p-3 bg-futuremilestone-accent/10 border border-futuremilestone-accent/20 rounded-[16px] flex items-center gap-2 text-futuremilestone-ink text-[12px]">
              <span className="text-futuremilestone-accent font-bold text-[14px]">💡</span>
              <span>
      ...
      <section className="p-[18px] sm:p-[22px] bg-futuremilestone-panel/72 border border-futuremilestone-soft-line backdrop-blur-[14px] rounded-[32px] shadow-futuremilestone-soft">
        <div className="flex items-center justify-between gap-4 mb-4">
          ...
          <button
            ...
            className={`relative inline-flex h-6.5 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
              form.carouselVisible ? "bg-futuremilestone-accent" : "bg-futuremilestone-ink/10"
            }`}
          >
            <span
              className={`pointer-events-none inline-block h-5.5 w-5.5 transform rounded-full bg-futuremilestone-panel-strong shadow ring-0 transition duration-200 ease-in-out ${
        ...
        {form.carouselVisible && (
          <div className="grid gap-4 mt-2">
            <div className="border-t border-futuremilestone-ink/5 pt-4">
              <div className="flex items-center justify-between gap-4 mb-3">
                <h3 className="text-[15px] font-bold text-futuremilestone-ink mb-0">Carousel Slides</h3>
                <button
                  type="button"
                  onClick={addSlide}
                  className="rounded-full px-[14px] py-1.5 border border-futuremilestone-line bg-futuremilestone-panel-strong text-futuremilestone-ink font-semibold text-center transition hover:bg-futuremilestone-accent hover:text-futuremilestone-bg active:scale-[0.98] cursor-pointer inline-block text-[12px]"
                >
                  Add Slide
                </button>
              </div>
              <div className="grid grid-cols-1 gap-4">
                {form.slides.map((slide: CarouselSlide, idx: number) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="p-4 bg-futuremilestone-panel/50 border border-futuremilestone-soft-line rounded-[24px] grid grid-cols-1 lg:grid-cols-12 gap-4 relative"
                    >
                      <div className="lg:col-span-12 flex items-center justify-between border-b border-futuremilestone-ink/5 pb-2 mb-1">
                        <span className="text-[13px] font-bold text-futuremilestone-ink">Slide {idx + 1}</span>
                        ...
                        >
                          {isUploading ? (
                            <div className="flex flex-col items-center justify-center gap-2">
                              ...
                              <span className="text-futuremilestone-muted text-[11px] font-medium">
                                Uploading...
                              </span>
                            </div>
                          ) : slide.bgImage ? (
                            <div className="absolute inset-0 w-full h-full group">
                              <img
                                ...
                                src={slide.bgImage}
                              />
                              <div className="absolute inset-0 bg-futuremilestone-ink/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2 backdrop-blur-[2px]">
                                <button
                                  type="button"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                  ...
                                  className="px-3 py-1.5 bg-futuremilestone-panel-strong text-futuremilestone-ink rounded-full text-[12px] font-semibold hover:bg-futuremilestone-accent hover:text-futuremilestone-bg transition duration-200 shadow-md"
                                >
                                  Change image
                                </button>
                                <button
                            ...
                            <div className="flex flex-col items-center justify-center text-center gap-2">
                              <CloudUploadIcon className="w-5 h-5 text-futuremilestone-accent" />
                              <span className="text-[12px] font-semibold text-futuremilestone-ink">
                                Upload slide image
                              </span>
                            </div>
                          )}
                      ...
                      <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="grid gap-1.5">
                          <label className="text-[12px] font-semibold text-futuremilestone-ink" htmlFor={`slide-name-${idx}`}>
                            Slide Title / Name
                          </label>
                          <input
                            id={`slide-name-${idx}`}
                        ...
                        <div className="grid gap-1.5">
                          <label className="text-[12px] font-semibold text-futuremilestone-ink" htmlFor={`slide-tagline-${idx}`}>
                            Tagline
                          </label>
                          <input
                        ...
                        <div className="grid gap-1.5">
                          <label className="text-[12px] font-semibold text-futuremilestone-ink" htmlFor={`slide-price-${idx}`}>
                            Starting Price
                          </label>
                          <input
                            id={`slide-price-${idx}`}
                        ...
                        <div className="grid gap-1.5">
                          <label className="text-[12px] font-semibold text-futuremilestone-ink" htmlFor={`slide-slug-${idx}`}>
