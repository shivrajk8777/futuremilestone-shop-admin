...
export default function DeliveryPartnerForm({ partner, isEdit = false }: DeliveryPartnerFormProps) {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6 max-w-2xl">
      ...
      <section className="p-[18px] sm:p-[22px] bg-futuremilestone-panel/72 border border-futuremilestone-soft-line backdrop-blur-[14px] rounded-[32px] shadow-futuremilestone-soft flex flex-col gap-5">
        <div className="mb-1">
          ...
          <p className="text-futuremilestone-muted text-[14px] mt-1">Identify the courier and choose a signature icon.</p>
        </div>
            ...
            {LOGO_OPTIONS.map((opt) => (
              <button
                ...
                className={`w-12 h-12 rounded-2xl border text-xl flex items-center justify-center cursor-pointer transition-all hover:scale-105 active:scale-[0.97] ${
                  form.logo === opt
                    ? "bg-futuremilestone-accent border-transparent text-futuremilestone-bg shadow-sm"
                    : "bg-futuremilestone-input-bg border-futuremilestone-ink/10 hover:border-futuremilestone-ink/20"
                }`}
              >
                {opt}
              </button>
        ...
        {/* Status Toggle */}
        <div className="flex items-center justify-between border-t border-futuremilestone-soft-line pt-5 mt-2">
          <div>
            <h3 className="text-[15px] font-bold">Active Status</h3>
            <p className="text-futuremilestone-muted text-[13px] mt-0.5">Toggle this courier availability on storefront checkout.</p>
          </div>
          <button
            type="button"
            ...
            className={`relative inline-flex h-6.5 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
              form.active ? "bg-futuremilestone-accent" : "bg-futuremilestone-ink/10"
            }`}
          >
            <span
