...
export interface ContactReplyFormProps {
  contactId: string;
  contactName: string;
  contactEmail: string;
}

export function ContactReplyForm({ contactId, contactName, contactEmail }: ContactReplyFormProps) {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
      ...
      <div className="grid gap-3">
        <div className="grid gap-1">
          <label className="text-[10px] font-bold text-futuremilestone-muted uppercase tracking-wider pl-1.5">
            Subject
          </label>
          <input
            type="text"
            ...
            disabled={sending}
            className="w-full border border-futuremilestone-ink/10 rounded-xl bg-futuremilestone-input-bg px-4 py-3.5 text-futuremilestone-ink outline-none text-xs focus:border-futuremilestone-ink/25 focus:ring-4 focus:ring-futuremilestone-ink/6 font-medium disabled:opacity-60"
          />
        </div>
        <div className="grid gap-1">
          <label className="text-[10px] font-bold text-futuremilestone-muted uppercase tracking-wider pl-1.5">
          ...
          <textarea
            ...
            disabled={sending}
            className="w-full border border-futuremilestone-ink/10 rounded-xl bg-futuremilestone-input-bg px-4 py-3.5 text-futuremilestone-ink outline-none text-xs focus:border-futuremilestone-ink/25 focus:ring-4 focus:ring-futuremilestone-ink/6 resize-none font-medium disabled:opacity-60"
          />
        </div>
      </div>
      ...
      <div className="flex items-center justify-end gap-2 pt-1">
        <button
          type="submit"
          disabled={sending}
          className="rounded-full px-5 py-2.5 bg-futuremilestone-accent text-futuremilestone-bg font-semibold text-[11px] hover:bg-opacity-95 active:scale-[0.98] transition-all cursor-pointer disabled:opacity-60 flex items-center gap-1.5"
        >
          {sending ? (
            <>
