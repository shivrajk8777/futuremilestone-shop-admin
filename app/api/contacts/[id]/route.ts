...
export async function PUT(request: NextRequest, { params }: ContactRouteParams): Promise<NextResponse> {
  try {
    ...
    const emailHtml = `
      <div style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1e293b;">
        ...
        <div style="border-top: 1px solid #e2e8f0; padding-top: 16px; font-size: 12px; color: #94a3b8; text-align: center;">
          <p style="margin: 0 0 4px;">Future Milestone Studio</p>
...
export async function DELETE(request: NextRequest, { params }: ContactRouteParams): Promise<NextResponse> {
  try {
    const { id } = await params;
    const objId = parseId(id);
    if (!objId) {
