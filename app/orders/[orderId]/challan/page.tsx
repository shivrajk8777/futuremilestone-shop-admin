...
export default async function OrderChallanPage({ params }: OrderChallanPageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-black p-4 md:p-8 font-sans select-text relative">
      ...
      <div id="challan-printable-card" className="max-w-[850px] mx-auto bg-white border border-gray-300 p-8 md:p-12 shadow-sm rounded-2xl print:border-none print:shadow-none print:p-0 print:m-0">
        ...
        <div className="flex justify-between items-start gap-6 border-b-2 border-black pb-6 mb-8">
          ...
          <div className="text-right">
            <span className="px-3 py-1 bg-black text-white text-[10px] uppercase font-black tracking-wider rounded-md inline-block mb-2">
              OFFICIAL DELIVERY CHALLAN
            </span>
            <h2 className="text-base font-bold text-gray-900 font-mono">{challanNo}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 border-b border-gray-200 pb-6">
          <div>
            <h3 className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-2">Consignee (Ship To)</h3>
            <div className="text-[12.5px] leading-relaxed">
              <p className="font-bold text-gray-900">{recipientName}</p>
              <p className="text-gray-700 mt-0.5">{addressLine}</p>
          ...
          <div>
            <h3 className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-2">Logistics & Handling</h3>
            <div className="text-[12.5px] space-y-1">
              <p><strong>Carrier Partner:</strong> {order.deliveryPartnerName || "Standard Shipping"}</p>
              <p><strong>Tracking / AWB:</strong> <span className="font-mono font-bold bg-gray-100 px-1.5 py-0.5 rounded text-indigo-700">{order.trackingId || "Assigned in Transit"}</span></p>
              {order.transactionId && (
        ...
        <div className="mb-8">
          <h3 className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-3">Itemized Shipment Manifest</h3>
          <table className="w-full border-collapse text-left text-[12.5px]">
            <thead>
              <tr className="border-b-2 border-gray-200 text-gray-700 font-bold uppercase text-[10.5px]">
                <th className="py-2.5">Item & Specs</th>
                <th className="py-2.5">Variant Details</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {order.items.map((item: any, idx: number) => {
                ...
                return (
                  <tr key={`${item.slug || item.name}-${idx}`} className="align-top">
                    <td className="py-3">
                      <div className="flex items-center gap-3">
                        <img
                    ...
                    <td className="py-3 text-gray-500 text-[11.5px] capitalize">
                      {[item.material, item.dimension].filter(Boolean).join(" • ") || "Standard Finish"}
                    </td>
      ...
      <style dangerouslySetInnerHTML={{
        ...
        @media print {
          * {
          ...
          #challan-printable-card, #challan-printable-card * {
            visibility: visible !important;
          }
          #challan-printable-card {
